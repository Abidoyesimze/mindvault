...
- **admin** — set via `nominate_new_admin` (see above). Can grant/revoke the verifier role (`add_verifier`/`remove_verifier`), set the registry fee config and burn/charity destination policy (`set_fee_config`/`set_fee_destination`), repair the pagination index (`repair_index`) or tag index (`repair_tag_index`). Cannot mutate any resource's price, metadata, listing, tags, or ownership.
